...
class Utils
{
	...
	public static function notionToMarkdown($notion)
	{
		...
		foreach ($notion as $item) {
			...
			if ((in_array("c", $format_types) or in_array("a", $format_types)) and (in_array("b", $format_types) or in_array("i", $format_types)) and empty($trailing_whitespace)) {
				$markdown .= " ";
			}
		...
		foreach ($markdown_chunks as $i => $chunk) {
			$prev = isset($markdown_chunks[$i - 1]) ? $markdown_chunks[$i - 1] : '';
			$curr = $chunk;
			$next = isset($markdown_chunks[$i + 1]) ? $markdown_chunks[$i + 1] : '';

			$prev_ended_in_delimiter = in_array(substr($prev, -1), self::delimiters);
			$next_starts_with_delimiter = in_array(substr($next, 0), self::delimiters);

			if ($prev_ended_in_delimiter and $next_starts_with_delimiter and !$last_used_underscores and substr($curr, 0, 1) == "☃" and substr($curr, -1, 1) == "☃") {
				if (substr($curr, 1, 1) == "☃") {
					$count = 2;
				} else {
					$count = 1;
				...
				$curr = str_pad(substr($curr, $count, strlen($curr) - $count * 2), $count, "_", STR_PAD_BOTH);
			...
			if (strpos($final_markdown, "***") !== false) {
